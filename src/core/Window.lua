...
function Window.new(opts, screenGui, flags, tier)
    ...
    tierBadgePad.PaddingLeft   = UDim.new(0, 4)
    tierBadgePad.PaddingRight  = UDim.new(0, 4)
    tierBadgePad.Parent        = tierBadge

    local tierBadgeCorner = Instance.new("UICorner")
    tierBadgeCorner.CornerRadius = UDim.new(0, 3)
    tierBadgeCorner.Parent       = tierBadge

    local tierBadgeStroke = Instance.new("UIStroke")
    tierBadgeStroke.Thickness = 1
    tierBadgeStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
    tierBadgeStroke.Parent    = tierBadge

    local tierBadgeLabel = Instance.new("TextLabel")
    tierBadgeLabel.BackgroundTransparency = 1
    tierBadgeLabel.Size                   = UDim2.new(1, 0, 1, 0)
    tierBadgeLabel.Text                   = ""
    tierBadgeLabel.Font                   = Enum.Font.GothamBold
    tierBadgeLabel.TextSize               = 10
    tierBadgeLabel.TextXAlignment         = Enum.TextXAlignment.Center
    tierBadgeLabel.Parent                 = tierBadge

    local TIER_STYLES = {
        Freemium = { text = "Freemium", color = Color.fromHex("#60a5fa") },
        Premium  = { text = "Premium",  color = Color.fromHex("#a855f7") },
    }
    ...
    local function applyTierStyle(t)
        ...
        if style then
            ...
            tierBadgeLabel.Text        = style.text
            tierBadgeLabel.TextColor3  = style.color
            tierBadgeStroke.Color      = style.color
        else
