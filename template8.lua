--[[
    ...
    v8 — PlayerGui fix, Roblox menu di atas window
...
MuvaUI:SetConfigSystem({ File="myscript_configs.json", Slots=5 })

local win = MuvaUI:CreateWindow({ Title="My Script", SubTitle="by MuvaUI", Version="v1.0.0" })
...
hstack:AddButton({ Title="Execute", Style="Default", Width=85, Callback=function() MuvaUI:Notify({Title="Executed",Body="Script running",Type="success",Duration=3}) end })
hstack:AddButton({ Title="Pause",   Style="Warn",    Width=85, Callback=function() MuvaUI:Notify({Title="Paused",Body="Script paused",Type="warn",Duration=2})    end })
hstack:AddButton({ Title="Stop",    Style="Danger",  Width=85, Callback=function() MuvaUI:Notify({Title="Stopped",Body="Scripts stopped",Type="error",Duration=3})  end })

local secVStack = tabLayout:AddSection({ Title="VStack" })
local vstack = secVStack:AddVStack({ Gap=6 })
vstack:AddToggle({ ID="VStack_T1", Title="Option A", Default=true,  Callback=function(v) print("[VStack] A →", v) end })
vstack:AddToggle({ ID="VStack_T2", Title="Option B", Default=false, Callback=function(v) print("[VStack] B →", v) end })
vstack:AddSlider({ ID="VStack_S1", Title="Intensity", Min=1, Max=10, Step=1, Default=5, Callback=function(v) print("[VStack] Intensity →", v) end })
...
accordion:AddToggle({ ID="Acc_T1", Title="Debug Mode", Default=false, Callback=function(v) print("[Accordion] Debug →", v) end })
accordion:AddSlider({ ID="Acc_S1", Title="Timeout", Min=1, Max=30, Step=1, Default=10, Suffix="s", Callback=function(v) print("[Accordion] Timeout →", v) end })
...
secDialog:AddButton({ Title="Open Confirm Dialog", Style="Default", Callback=function()
    win:Dialog({
        ...
        Buttons={
            { Text="Confirm", Style="Danger", Callback=function() MuvaUI:Notify({Title="Confirmed",Body="Action executed",Type="success",Duration=2}) end },
...
secAppearance:AddSlider({ ID="UIOpacity", Title="UI Opacity", Min=50, Max=100, Step=5, Default=100, Suffix="%", Callback=function(v) print("[Settings] Opacity →", v) end })
...
print("[MuvaUI] Template v8 loaded. All components registered.")
